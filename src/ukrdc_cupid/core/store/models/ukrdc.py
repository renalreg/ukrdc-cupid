...
class Node(ABC):
    ...
    def add_item(self, property:str, value: Union[str,XmlDateTime], is_optional:bool=True):
    ...
    def add_children(self, child_node: Node, xml_items: xsd_ukrdc, attr_name:str=None):
...
class LabOrder(Node):
    ...
    def map_xml_to_tree(self):
        ...
        self.add_code("receivinglocationcode", "receivinglocationdesc", "receivinglocationcodestd",self.xml.receiving_location)
        self.add_code("orderedbycode", "orderedbydesc", "orderedbycodestd", self.xml.ordered_by)
        self.add_code("orderitemcode", "orderitemdesc", "orderitemcodestd", self.xml.order_item)
        self.add_code("ordercategorycode", "ordercategorydesc", "ordercategorycodestd", self.xml.order_category)

        self.add_code("prioritycode", "prioritydesc", "prioritycodestd", self.xml.priority)
        self.add_code("patientclasscode", "patientclassdesc", "patientclasscodestd", self.xml.patient_class)
        self.add_code("enteredatcode", "enteredatdesc", "enteredatcodestd", self.xml.entered_at)
        self.add_code("enteringorganizationcode", "enteringorganizationdesc", "enteringorganizationcodestd", self.xml.entering_organization )
...
class Patient(Node):
    ...
    def add_person_to_contact(self, xml:xsd_types.PersonalContactType):
        ...
        if xml:
            ...
            if xml.contact_details:
                ...
                self.orm_object.persontocontact_contactnumber = self.xml.contact_details[0].value
                self.orm_object.persontocontact_contactnumbercomments = self.xml.contact_details[0].comments
                self.orm_object.persontocontact_contactnumbertype = self.xml.contact_details[0].use
    ...
    def map_xml_to_tree(self):
        ...
        self.add_code("ethnicgroupcode","ethnicgroupstd", "ethnicgroupdesc", self.xml.ethnic_group)
