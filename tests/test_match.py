...
def _test_overwrite_with_chi_no(ukrdc_test: Session):
...
def test_overwrite_with_chi_no(ukrdc_test: Session):
    """ I think I came up with some clever way to handle switching chi for nhs
    however I have rewritten the function so for now we will just check it
    raises an error.
    """
    xml_path = os.path.join("tests", "xml_files", "store_tests", "test_3.xml")
    xml_test = load_xml_from_path(xml_path)
